  ...
  for(j in 1:n_dim_filter[i]){
    ...
    EOF_time_series_plot <- ggplot(EOF_PC_spp_2,
      ...
      geom_vline(xintercept=EOF_PC_spp_2$Year_Month[which(str_detect(EOF_PC_spp_2$Year_Month,"_01"))],
                 linetype="dashed", color = "skyblue", size = 1)+
      geom_hline(yintercept=0,linetype="dashed", color = "grey", size = 1)+
      geom_line(size=1,color=color_name[j])+
      theme_bw()+
      xlab("")+ylab("")+
      theme()+
      ...
      geom_text(aes(x = max(EOF_PC_spp$Year_Month),
                    y = -Inf, label = "2018"),
...
--> seasonnal PC, coincide with reproduction ecology
  --> result from migration patterns ?
